...
const LocationPopup = ({
  ...
  const {
    isOpen: ctxIsOpen,
    setIsOpen: ctxSetIsOpen,
  } = useCityFilter();

  const inputRef = useRef();

  // Blur input on mobile to avoid unwanted keyboard pop-up
  useEffect(() => {
  ...
  /* --------------------------------------------------------------- */
  /*  Common wrapper – decides left-aligned vs centered               */
  /* --------------------------------------------------------------- */
  const Wrapper = ({ children, className = "" }) => {
    if (forceCenter) {
      /*  ----  CENTERED (Home domain)  ----  */
      return (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center">
          <div
            className={`bg-white w-full md:w-[75%] max-w-3xl rounded-t-lg md:rounded-lg shadow-xl ${className}`}
          >
            {children}
          </div>
        </div>
    ...
    /*  ----  ORIGINAL LEFT-ALIGNED (Venue, etc.)  ----  */
    return (
      <div className="fixed inset-0 z-50 bg-black/60">
        <div
          className={`fixed md:hidden block top-0 left-1/2 transform -translate-x-1/2 w-full h-[100vh] bg-white border-t border-gray-300 shadow-md z-50 ${className}`}
        >
          {children}
        </div>

        {/* Desktop left-aligned version (kept for backward compatibility) */}
        <div
          className={`hidden md:block fixed top-15 left-1/2 transform -translate-x-1/2 w-[75%] min-h-max bg-white border-t border-gray-300 shadow-md z-50 ${className}`}
        >
          {children}
        </div>
      </div>
    ...
    <Wrapper className="">
      ...
      <div className="flex items-center rounded-md px-3 py-2 mx-3 bg-gray-50 border border-gray-200 my-2">
        <input
          ...
          className="flex-1 text-[14px] outline-none text-gray-700 bg-transparent"
          ref={inputRef}
          autoFocus={!forceCenter}      
        />
        <FiSearch className="text-gray-400 ml-2" />
      </div>

      {/* ---------- LIST ---------- */}
      <div className="pb-3 h-[70vh] md:h-auto overflow-y-auto scrollbar-hide">
        {/* POPULAR CITIES */}
        <div className="px-4">
          <h3 className="text-center text-[14px] font-medium text-gray-600 mb-2">
            Popular Cities
          </h3>
        ...
        {/* OTHER CITIES (filtered) */}
        {filteredCities.length > 0 && (
          <div className="px-4 py-2 border-t border-gray-100">
            <h3 className="text-center text-[14px] font-medium text-gray-600 mb-2">
              Other Cities
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 max-h-[250px] overflow-y-auto scrollbar-hide">
              {filteredCities.map((city) => (
                <div
                  key={city.id}
                  className="px-3 py-2 text-gray-700 text-[13px] bg-gray-50 rounded hover:bg-gray-100 cursor-pointer text-center"
        ...
        {filteredCities.length > 0 && !forceCenter && (
          <div className="flex justify-center mt-3">
            <button

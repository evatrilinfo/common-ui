...
//     { name: 'Home', path: storedCity ? `/${storedCity.slug}` : 'https://evatril.com', icon: <FaHome className='text-lg mb-1' /> },
//     { name: 'Venue', path: 'https://venue.evatril.com/', icon: <FaUtensils className='text-lg mb-1' /> },
//     { name: 'Menu', path: 'https://menu.evatril.com/', icon: <FaUtensils className='text-lg mb-1' /> },
//     { name: "Invitation", icon: <FaEnvelopeOpenText className="text-lg mb-1" />, path: "https://curesoon.in" },
//     { name: 'Profile', path: '/profilepage', icon: <FaUser className='text-xl mb-1' />, mobileOnly: true },
//   ];
...
//     // Cross-domain navigation (Menu, Venue, etc.)
//     window.location.assign(tab.path);
//   } 
//   else {
//     // Internal navigation (same domain routes)
//     navigate(tab.path);
//   }
...
//   const isSubNav = currentPath.includes('/subnavcards');
//   const isCatagoryCard = currentPath.includes('/catagoryCardDetails');
//   const isCardDetails = matchPath('/venuedetails/card-details/:slug', currentPath) || matchPath('/:citySlug/venuedetails/card-details/:slug', currentPath);
//   const isProfile = currentPath.includes('/profilepage');
//   const isBooking = matchPath('/venues/checkout/:slug', currentPath);
//   const isCheckout = matchPath('/venuedetails/checkout/:slug', currentPath) || matchPath('/:citySlug/venuedetails/checkout/:slug', currentPath) || matchPath('/:citySlug/venuedetails/card-details/checkout/:slug', currentPath) || matchPath('/venuedetails/card-details/checkout/:slug', currentPath);
//   const isAllVenues = currentPath.includes('/venues');
//   const isDestinationPage = currentPath.includes('/destination-venues');
...
//       {/* Bottom Navigation with Dynamic z-index */}
//       <div className={`fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 shadow-md flex justify-around items-center md:hidden z-[${getZIndex()}] h-[60px]`}>
//         {tabs.map((tab) => (
//           <button
//             key={tab.name}
//             className={`flex flex-col items-center justify-center text-[12px] md:text-[14px] ${activeTab === tab.name ? 'text-red-600' : 'text-gray-500'}`}
...
//   const tabs = [
//     { name: 'Home', path: 'https://evatril.com', icon: <FaHome className='text-lg mb-1' /> }, // Always go to root domain
//     { name: 'Venue', path: 'https://venue.evatril.com/', icon: <FaUtensils className='text-lg mb-1' /> },
//     { name: 'Menu', path: 'https://menu.evatril.com/', icon: <FaUtensils className='text-lg mb-1' /> },
//     { name: "Invitation", icon: <FaEnvelopeOpenText className="text-lg mb-1" />, path: "https://curesoon.in" },
//     { name: 'Profile', path: '/profilepage', icon: <FaUser className='text-xl mb-1' />, mobileOnly: true },
...
import {
  fetchCities,
  setSelectedCity,
} from '../store/Slices/CitySlice/citySlice';
import {
  FiUser,
  FiSearch,
  FiX,
...
import { RxCross2 } from 'react-icons/rx';
import SearchPopup from './SearchPopup';
import LocationPopup from './LocationPopup';          // <-- same folder
import NavComponent from './NavComponent';
import { matchPath } from 'react-router-dom';
import { useCityFilter } from '../Context/CityFilterContext';
import {
...
const Navbar = () => {
  ...
  /* ------------------------------------------------------------------ */
  /*  Determine active tab (Home / Venue / Menu …)                     */
  /* ------------------------------------------------------------------ */
  useEffect(() => {
    const hostname = window.location.hostname;
    const pathname = location.pathname;
  ...
  const getZIndex = () => {
    const map = { Home: '9', Venue: '99', Menu: '11', Invitation: '12', Profile: '13' };
    return map[activeTab] || '9';
  };

  /* ------------------------------------------------------------------ */
  /*  Scroll handling for sort/filter bar (catagoryCardDetails)        */
  /* ------------------------------------------------------------------ */
  useEffect(() => {
    if (currentPath === '/catagoryCardDetails') {
      const handle = () => {
        const cur = window.scrollY;
        if (cur > lastScrollY.current && cur > 50) setShowSortFilter(true);
        else if (cur < lastScrollY.current) setShowSortFilter(false);
  ...
  /* ------------------------------------------------------------------ */
  useEffect(() => {
    const lock = isOpen || isMobileMenuOpen || showLocationPopup || showSearchInput;
  ...
  /* ------------------------------------------------------------------ */
  const tabs = [
    { name: 'Home', path: 'https://evatril.com', icon: <FaHome className="text-lg mb-1" /> },
    { name: 'Venue', path: 'https://venue.evatril.com/', icon: <FaUtensils className="text-lg mb-1" /> },
    { name: 'Menu', path: 'https://menu.evatril.com/', icon: <FaUtensils className="text-lg mb-1" /> },
    { name: 'Invitation', path: 'https://curesoon.in', icon: <FaEnvelopeOpenText className="text-lg mb-1" /> },
    { name: 'Profile', path: '/profilepage', icon: <FaUser className="text-xl mb-1" />, mobileOnly: true },
  ];
  ...
  const handleTabClick = (tab) => {
    ...
    if (tab.path?.startsWith('http')) window.location.assign(tab.path);
  ...
  const handleNavigateSearch = () => setShowSearchInput(true);
  const handleNavigateNotification = () => navigate('/notification');
  ...
  const isCardDetails = matchPath('/venuedetails/card-details/:slug', currentPath) || matchPath('/:citySlug/venuedetails/card-details/:slug', currentPath);
  const isProfile = currentPath.includes('/profilepage');
  const isCheckout = matchPath('/venues/checkout/:slug', currentPath) ||
    matchPath('/venuedetails/checkout/:slug', currentPath) ||
    matchPath('/:citySlug/venuedetails/checkout/:slug', currentPath) ||
    matchPath('/:citySlug/venuedetails/card-details/checkout/:slug', currentPath) ||
    matchPath('/venuedetails/card-details/checkout/:slug', currentPath);
  const isAllVenues = currentPath.includes('/venues');
  const isDestinationPage = currentPath.includes('/destination-venues');
  const isLocation = currentPath.includes('/locationpage');

  /* ------------------------------------------------------------------ */
  /*  Back navigation (keeps original location.state.from)            */
  /* ------------------------------------------------------------------ */
  const initialFromRef = useRef(location.state?.from || null);
  useEffect(() => {
    if (!initialFromRef.current && location.state?.from) {
  ...
  const handleBack = () => {
    if (initialFromRef.current) navigate(initialFromRef.current);
    else navigate(-1);
  };

  /* ------------------------------------------------------------------ */
  /*  NEW: decide whether the popup must be forced-centered            */
  /* ------------------------------------------------------------------ */
  const isHomeDomain = window.location.hostname.includes('evatril.com') && !window.location.hostname.includes('venue');
  ...
  return (
    <>
      {/* ------------------------------------------------------------------ */}
      {/*  Secondary pages – use NavComponent + mobile back bar               */}
      {/* ------------------------------------------------------------------ */}
      {(isVenueDetails || isSubNav || isCatagoryCard || isCardDetails || isProfile || isCheckout || isDestinationPage || isAllVenues || isLocation) ? (
        <>
          <NavComponent
          ...
          <nav className="fixed top-0 left-0 w-full h-[55px] text-gray-700 font-bold text-[16px] sm:pl-2 flex md:hidden items-center shadow-sm md:px-10 lg:px-10 px-4 py-3.5 z-[99] bg-white backdrop-blur-md">
            ...
            <p className="font-normal pl-2 text-black">
              {currentPath === '/subnavcards'
                ...
                : isDestinationPage
                ? 'Destination Wedding'
                : currentPath === '/venues' || ':city/venues'
                ? `Venues`
                : currentPath === '/locationpage'
        ...
        <nav className="fixed top-0 left-0 w-full h-[55px] md:h-[65px] flex space-x-0 items-center shadow-sm md:px-6 lg:px-10 px-3 py-3 z-[99] bg-white backdrop-blur-md">
          <div className="flex items-center w-full justify-between md:justify-between">
            ...
            {/* RIGHT SIDE ICONS + CITY DROPDOWN */}
            <div className="flex items-center justify-end md:space-x-3 lg:space-x-4">
              {/* SEARCH ICON */}
              <div className="hidden md:flex">
                <div
              ...
              <div className="hidden md:flex lg:flex-col relative w-[120px] h-10" ref={dropdownRef}>
                <div
                  className="nav-dropdown-city flex items-center rounded-md px-2 justify-center py-2 text-gray-700 cursor-pointer active:bg-gray-100 active:scale-99 transition duration-100"
                  onClick={() => setTimeout(() => toggleDropdown(), 120)}
                >
              ...
              <div className="flex md:hidden items-center justify-end gap-3.5">
                ...
                {/* MOBILE MENU DRAWER */}
                <div
                  className={`fixed top-0 right-0 h-full w-[100vw] shadow-lg z-40 transition-transform duration-300 ease-in-out transform ${
      ...
      {/* ------------------------------------------------------------------ */}
      {/*  LOCATION POPUP – now receives forceCenter prop                     */}
      {/* ------------------------------------------------------------------ */}
      {(isOpen || showLocationPopup) && (
        <LocationPopup
          forceCenter={isHomeDomain}               // <--- NEW PROP
          setIsOpen={setIsOpen}
          onClose={() => {
            setIsOpen(false);
            setShowLocationPopup(false);
      ...
      {/* ------------------------------------------------------------------ */}
      {/*  SORT/FILTER BAR (catagoryCardDetails)                             */}
      {/* ------------------------------------------------------------------ */}
      {currentPath === '/catagoryCardDetails' && (
        <div
          className={`fixed left-0 w-full bg-white border-t border-gray-200 shadow-md flex justify-around items-center md:hidden z-[99] h-[55px] transition-transform duration-300 ease-in-out ${
            showSortFilter ? 'translate-y-0' : 'translate-y-[60px]'
          }`}
        >
          {sortFilterTabs.map((tab) => (
            <button
      ...
      {/* ------------------------------------------------------------------ */}
      {/*  BOTTOM NAV (mobile) – dynamic z-index                              */}
      {/* ------------------------------------------------------------------ */}
      <div
        className={`fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 shadow-md flex justify-around items-center md:hidden z-[${getZIndex()}] h-[60px]`}
      >
        {tabs.map((tab) => (
          <button
            key={tab.name}

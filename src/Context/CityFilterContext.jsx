...
export const CityFilterProvider = ({ 
  children, 
  onCityChange, // New prop for handling city changes
  currentPath = '/', // Current path from main app
  currentSearch = '' // Current search params from main app
}) => {
  const dispatch = useDispatch();
  const { selectedCity, cities, loading: citiesLoading } = useSelector((state) => state.city);
  ...
  useEffect(() => {
    const pathSlug = currentPath.split('/')[1];
  ...
  }, [currentPath, currentSearch, cities, selectedCity, dispatch]);
  ...
  const handleCitySelect = (city) => {
    ...
    // Call the callback with navigation data instead of navigating directly
    if (onCityChange) {
      const searchParams = new URLSearchParams(currentSearch);
      let newPath;
